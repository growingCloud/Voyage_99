...
# 회원에게만 글 작성 권한 부여
@login_required                                         # (2) 하지만 모든 페이지에 이렇게 구현하기에 비효율적이라 데코레이터 기능을 사용한다
def board_write():
...
def board_lists():
    ...
    limit = request.args.get("limit", 5, type=int)    # 한 페이지 당 몇개의 게시물을 가져올지
    ...
    total_count = db.board.find(query).countDocuments()        # 게시물의 총 갯수
    ...
    block_num = int((page - 1) / block_size)                # 현재 게시글 블럭의 위치 : 지금 몇 페이지 블럭에 위치해 있는가?
